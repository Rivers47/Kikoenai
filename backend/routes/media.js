        ...
        getTrackList(req.params.id, path.join(rootFolder.path, work.dir), JSON.parse(work.memo))
          .then(async (tracks) => {
            ...
            try {
              ...
              // track.contentHash comes from the work's cached memo, which
              // GET /api/tracks/:id fills in the first time the work is opened;
              // fall back to hashing here so this route also works before that
              // has happened, and for a file left unhashed because it could
              // not be read.
