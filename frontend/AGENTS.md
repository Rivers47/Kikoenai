...
2. **Work page tabs + cover gallery:** `Work.vue` puts the file tree and the scraped description in `q-tabs` + `q-tab-panels`, with the open tab in the url as `?tab=description` (absent = files). **Not `q-route-tabs`** — its active state is matched on the path, which is identical for both tabs, so neither would ever highlight; that would take real child routes. The panels are `keep-alive`, and the tree's own folder/image state already lives in `?path=`/`?img=`, so switching tabs keeps the open folder and its scroll position. The tab bar only renders when the work has a description at all. The cover itself is `WorkGallery.vue` — a `q-carousel` over the cover plus every **downloaded** scraped image (see `workImageUrl`), with arrows and an `n/N` counter; a work with nothing downloaded shows just the cover and no counter. `Cover.vue` is untouched and still used by the cards, lists and player; its id/release overlays have no room in the carousel, so the work page prints them next to the title instead.
