...
│   │   ├── Cover.vue                     # Cover image + id/release/tag chips
...
```javascript
{
  ...
  sleepStoppedTrackId: '',          // trackId a tracks-mode timer stopped on; next play advances past it (§2.7.3)
  rewindSeekTime: 5,
  forwardSeekTime: 30,
...
3. **Sleep Timer:** `SleepMode.vue` (opened from the bedtime button in `AudioPlayer.vue`'s settings row) stops playback either after a chosen number of minutes (5-min slider steps) or after a chosen number of tracks past the current one. The armed timer is persisted in `SessionStorage` under the `sleepTimer` key (`SLEEP_TIMER_KEY`) and restored on reload; stop logic lives in `AudioElement.vue` (`onTimeupdate` for minutes, `onEnded` for tracks).
   ...
   **A tracks-mode stop leaves the queue parked on the track that just finished** — advancing there would report progress for a track the user never heard — so the advance is deferred to their next play (`_advanceIfSleepStopped`, called from the `playing` watcher). That leaves the player sitting on a media element that is already at its end, which is the one position browsers do not agree on: Chrome rewinds to 0 on the next `play()` (what the spec says an ended element does) and replays the finished track, while Firefox treats a seek to the end as the end of playback and advances by itself. **Never leave a resume parked there.** Two mechanisms keep it from happening, and they cover different ground:

   - **`SET_QUEUE` resolves it from the position alone.** The resume-from-history call sites (`RecentWorks.vue`, `FavListItem.vue`, `Work.vue`) pass `advancePastFinishedTrack: true`; a stored position within `FINISHED_EPSILON_SECONDS` of the parked track's `duration` resumes on the *next* track instead (or restarts the last track from 0, where there is nothing to advance to). This is the one that works **across devices and sessions** — the position comes from `t_track_progress`, so a timer that stopped on the phone is resumed the same way on the desktop — and it is the primary fix. It needs `duration` on the queue item, so a history row predating `toQueueItem` falls through to the marker below. Explicit track picks never pass the flag: choosing a finished track by hand must replay it.
   - **The marker covers the same device.** `sleepStoppedTrackId` is Vuex state, persisted in `SessionStorage` (`SLEEP_STOPPED_TRACK_KEY`) and keyed by **trackId, not queue index**, so it survives a reload — Chrome discards backgrounded tabs — and a queue rebuilt from history. `CLEAR_SLEEP_MODE` deliberately does **not** clear it: the stop clears the timer, and the marker has to outlive it. `onSeeked` drops it only for a seek that lands *inside* the track (`SEEK_END_TOLERANCE_SECONDS`) — scrubbing back means the user wants that track again, while a seek to the end is `onCanplay` applying a pending resume, not the user.

   `_advanceIfSleepStopped` also clears the pending `resumeHistorySeconds` and zeroes `currentTime` before advancing: both still describe the finished track, and `onCanplay` / the `queueIndex` watcher would otherwise apply them to the track being loaded.
