...
第一种错误是请求超时，一般稍后重试可以解决，如果屡次产生都失败，就要考虑是不是网络问题。需要注意的是，现在 DLsite 上许多内容只有日本 IP 才能访问，建议在 [高级设置] 页面中的 "爬虫相关设置" 中设置一个位于日本的代理后再扫描。
...
#### 8.关于如何启用 http 代理
现在 DLsite 上许多内容只有日本 IP 才能访问，建议使用位于日本的 http 代理进行抓取。
